...
### 2026-04-20 `3fec3c0` — 卡片「📋 簡報」pill

已完賽場次卡片 footer 多一顆米底赤陶字的「📋 簡報」pill，點擊展開 CPBL 賽事簡報（連勝連敗、個人里程碑）。

| 項目 | 決策 |
|------|------|
| 觸發條件 | `g.completed && g.sno && BRIEFINGS[g.sno]` — 只在已完賽且有簡報內容時顯示 |
| 位置 | `card-footer-buttons` 最右（購票/高鐵為未來場次，跟簡報互斥不會同時出現） |
