...
### 2026-04-28 — Phase 2-D 主程式實作（分支 `feat/phase2d-milestone-toast`）

把 Phase 2-D 從「設計收斂」推進到「主程式上線」。設計分支 `feat/phase2d-confetti-preview` 已 merge 進 main（`4b54f86`），此分支從 main 拉出寫主程式，4 個 commit 推完。

#### 實作分割

| Commit | 範圍 |
|--------|------|
| `466fe07` `feat(phase2d): schema v2 + 里程碑偵測函式（無 UI）` | `USER_STORE_VERSION` 升 2，加 `shownMilestones: {}` 與遷移；`detectMilestonesAfterCheckin()` 純邏輯 |
| `6602f4f` `feat(phase2d): 里程碑 toast HTML/CSS + 三類內容渲染` | L4 佈局 toast HTML/CSS、`getMilestoneContent()` 三類 ID → 顯示內容 mapping |
| `af7b4d1` `chore(phase2d): 動畫風格探索 preview` + `124f24b` `feat(phase2d): 進場/退場動畫 + 多里程碑排隊系統` | 三選一動畫探索（① 優雅 / ② 卡通彈跳 / ③ 派對狂歡）→ 選 ③；`enqueueMilestones` 排隊系統 |
| `85bd968` `feat(phase2d): 接線 saveCheckinForm 觸發里程碑 + 刪除倒退` | 接到實際打卡流程；刪除時 `cleanupShownMilestonesAfterDelete` 倒退 milestones |

#### 關鍵決策

| 項目 | 決策 |
|------|------|
| 場數門檻 | `CHECKIN_THRESHOLDS = [5, 10]`，**將來只往上加**（→ 25 / 50）。加更低值會讓老用戶補跳，要避開 |
| 里程碑 ID 命名 | `checkin-{N}` / `venue-{球場名}` / `venue-all` |
| 進場動畫 | ③ 派對狂歡：toast `scale 0.5→1.1→1` 500ms cubic-bezier 大彈跳 + confetti `scale 0→1.25→1` 700ms 延遲 150ms + `rotate -25°→0°` |
| 退場 / 防重播 | `msOverlayFadeOut` 200ms；`shownMilestones` map 阻擋重播；多 milestone 排隊間隔 300ms |
| 主隊晉級 | **本期不做**。CPBL 上下半季規則尚未在 code 中建模，且 2026 賽季 4 月才剛開季，最早 6 月底才有觸發機會。先做 ① 打卡達標、② 球場巡禮，③ 主隊晉級擱置 |
| 刪除語意 | **狀態反映**而非「成就一次解鎖永久」：刪到低於門檻就移除對應 `shownMilestones` key，下次達成可再跳。理由 — user 直接質疑「刪除不就應該是沒有要打卡嗎」，我之前推測「通常是修錯」沒根據 |
| 觸發點同源 | 三類偵測都掛在 `saveCheckinForm` 寫入後，避免散落各處 |

#### 開發教訓

- **「沒看到 toast」要先確認快取**：commit 2 user 開瀏覽器測 `showMilestoneToast()` 沒反應，原因是 `file://` 開檔被瀏覽器 cache 鎖住舊版（不是 code 問題）。下次 user 報「沒效果」優先請對方 `Ctrl+Shift+R`；本機開發必須走 HTTP server（`python -m http.server`），`file://` 還會撞 CORS 擋 fetch
- **不要替使用者腦補**：第一輪我寫「刪打卡通常是修錯不是真的沒去」推銷選 A（永久解鎖），user 直接反問「刪除不就應該是沒有要打卡嗎」。我承認沒根據後選 B（狀態反映）。教訓存進 memory `feedback_dont_project_assumptions_on_user.md`
- **動畫風格探索值得獨立 preview**：commit 3 前我用文字描述「scale 曲線 / confetti 整張 vs 拆碎片」user 看不懂，做了 `design/phase2d-animation-preview.html` 三變體並排可重播，user 看完一秒選 ③。能視覺比對的決策**不要靠文字溝通**
- **測試 artifact ≠ production bug**：`showMilestoneToast('venue-天母')` 在 console 直接呼叫時顯示「你已走訪 0 座球場」，正式流程不會（toast 觸發時 venue 已寫入 checkins）。但仍加 `Math.max(1, visited)` 保險（一行成本換永久不出醜）
- **Schema 升版用 spread 合併最省**：v1→v2 只加 `shownMilestones: {}`，不需寫遷移函式分支，`{ ...defaults, ...data, shownMilestones: { ...defaults.shownMilestones, ...(data.shownMilestones || {}) } }` 一行搞定。原來的 `migrateUserState` 結構已預先設計好

### 2026-04-27 下午 — Phase 2-D 成就 toast 設計探索（分支 `feat/phase2d-confetti-preview`，commit `82f01fb`，**未 merge / 未實作主程式**）

把 Phase 2-D 從「尚未開始」推進到「設計收斂完成」。本次只動 `design/` 與 `icon-preview/confetti-candidates/`，主程式 `index.html` / `cpbl-planner.html` 不動。

#### 設計決策

| 項目 | 決策 |
|------|------|
| confetti SVG | svgrepo `#325796` `party-confetti`（多色卡通風）— 與 `bb-favorite` / `bb-checkin` 同來源，免費商用 |
| toast 佈局 | L4：頂部 140px confetti 為主視覺，「成就解鎖」tag → h3 標題前置 28px inline icon → divider → desc → count pill |
| 球場巡禮 inline icon | Flaticon `stadium-484466.png`（既有素材，production 加 attribution） |
| 打卡達標 inline icon | `bb-checkin` sprite（與全站打卡按鈕語彙一致） |
| 主隊晉級 inline icon | `TEAM_LOGO` map 取使用者主隊 logo（CPBL 官方 CDN） |
| 動畫節奏 | 一次性：進場 scale 0.7→1.05→1（~400ms 卡通彈跳）+ confetti burst（~600ms）；之後靜止待 dismiss；fade-out 200ms |
| 防重播 | `shownMilestones` localStorage；多里程碑同步觸發採排隊間隔 ~300ms |

#### 設計 exploration 檔（4 份 HTML）

- `design/phase2d-confetti-preview.html` — **路線 1（自繪 SVG）失敗紀錄**：6 種自繪方向（光線放射 / 手繪緞帶 / 棒球周邊 / 小旗串 / 幾何粒子 / 棒球+星星）全部被 user 評為「都不太好看」。memory `feedback_no_freehand_complex_shapes.md` 再次驗證 — 我手繪複雜形狀本來就不行
- `design/phase2d-confetti-candidates.html` — **路線 2（svgrepo 精品）成功**：8 個現成候選縮圖檢視 + 套進 toast 上方的小型對照
- `design/phase2d-confetti-layouts.html` — 選定的 svgrepo SVG 套進 4 種佈局：L1 光環背景 / L2 拿掉徽章 / L3 縮小淡化 / L4 inline icon
- `design/phase2d-three-milestones.html` — L4 套三類里程碑驗證 inline icon 並排視覺平衡

#### 開發教訓

- **「不要 emoji」是視覺風格層，不是渲染技術層**：第一輪我把 user 的「不要 emoji」誤解成 uidesign.md L99 的「原生 emoji 跨平台不一致」規則，提了 Twemoji + Fluent 3D 當「替代方案」，當場被駁回 — 那兩個視覺上仍是 emoji。教訓存進 memory `feedback_no_emoji_includes_twemoji_fluent.md`
- **設計任務不要擅自修改任務外元素**：第一份 confetti preview 我順手把球場徽章從 Flaticon 立體建築 PNG 換成 `bb-checkin` 菱形 sprite，理由是 Flaticon 需要 attribution。但 (1) Flaticon 不是禁用素材、(2) 兩張圖意義不同（建築 vs 場地俯瞰）、(3) user 沒授權我動那塊。被指正「又偷懶沒驗證」。教訓存進 memory `feedback_no_unsolicited_fixes_in_design_task.md`
- **手繪 SVG 不是出路**：花了一輪做 6 個自繪方向才回到「該去既有素材庫」的結論。下次設計任務一開始就應該先列「(a) 用既有 sprite/icon-preview/、(b) 去 svgrepo / Flaticon 找精品、(c) 自繪」三條，自繪放最後 — 不要跳過 (a)(b) 直接動手畫
- **inline icon 用既有素材最省**：三類里程碑各自需要 inline icon，三個都從既有素材庫取（`stadium-484466.png` / `bb-checkin` sprite / `TEAM_LOGO` map），0 新增依賴。設計負擔最低、各 milestone 識別最清楚

### 2026-04-27 — Phase 1-D 打卡紀錄 sub-tab（分支 `feat/phase1d-checkins-tab`，merge `88bea42`）

把「我的 → 打卡紀錄」從 placeholder 換成正式列表，Phase 1-D 至此完整收尾。

#### 功能總覽

| 區塊 | 決策 |
|------|------|
| Row layout | `grid-template-columns: 72px 1fr` — 日期方塊 + 主內容；沿用收藏 tab 的 `月/日 / 週X` 日期 block 跟 `.fav-team` / `.fav-score` 對戰元件保持系列感，但拿掉收藏的右側 ★ 欄（打卡列不需要單獨 affordance，整列就是觸發器）|
| 主內容堆疊 | match → meta（球場 · ★ · 座位 chips）→ 同行 → 心得；同行/心得只在有值時 render，避免空殼撐高 |
| 摘要 pill | `.ck-summary` 跟徽章 tab `.vp-field-note` 同奶油底 + 赤陶圓點分隔風格保持一致；「最常去」只在 ≥2 次才出現 — 單筆打卡時講「最常去」是冗詞 |
| 星評樣式 | `.ck-stars` 5 顆 ★ 並排，未達等級用 `.off` 奶茶灰；不用 emoji ⭐ 因為 emoji 字寬不齊 |
| 座位 chips | `.ck-seat` 小奶油底圓角 + 赤陶紅字，跟延賽 pill 同色系；多選時並排 wrap |
| 心得 | `.ck-note` 奶油底 + 赤陶橙左 border，`white-space: pre-wrap` 保留使用者輸入的換行 |
| 互動 | 整列 `cursor: pointer` + `:focus-visible` 赤陶紅 outline；點/Enter/Space 都 call `openCheckinModal()`；走既有 modal 編輯/刪除流程，省一套全新 UI |
| 空狀態 | `bb-checkin` sprite 48×48 赤陶紅 — 不要 📍 emoji，跟打卡功能 icon 語彙一致（CLAUDE.md icon 命名表規則）|

#### 開發教訓

- **第一次處理使用者自由輸入文字**：之前 favorites/checkins 的 modal input 都靠 DOM property（`.value`）回填，這次首次以 HTML innerHTML 渲染 `note` / `companions` / `seat`。寫 `escapeHtml()` helper 統一通過，避免 `<script>` 直接注入。教訓：之前 codebase 沒有 escape helper 是因為沒這個需求，加新 feature 前要先檢查邊界條件。
- **重複的 match HTML 沒去抽公用**：`.ck-row` 的對戰 HTML 跟 `renderFavoritesList` 幾乎一樣（差別只在 `.fav-*` class name）。考量這次只有兩個地方共用 + DRY 後 class prefix 還是要 parameterize 反而更醜，刻意保留重複。教訓：過早抽象比重複還糟，但要在註解或 commit message 留下「下次再共用就抽」的訊號 — 這次靠 commit message + features.md 留痕。
- **「placeholder → 實作」最便宜的進場方式是延用既有元件**：這次完全沒新增 modal、沒新 schema、沒新 state — 點 row 直接打開原本打卡 modal 就 work，因為 modal 設計時已經有 `existing` 分支。教訓：Phase 1 schema 跟 Phase 1 modal 都預先想好「未來會再進來編輯」的入口，這次 sub-tab 收尾完全沒卡。

### 2026-04-23 下午 — Phase 1-D 收藏 sub-tab + 篩選 segment 重構（分支 `feat/phase1d-favorites-tab`）

補上原 Phase 1-D 設計的「收藏」sub-tab 內容（從 placeholder 升級為實際列表），同時把篩選區 2 個獨立 checkbox 重構為互斥 segment control。

#### 功能總覽

| 區塊 | 決策 |
|------|------|
| 收藏 sub-tab | row 格式（日期 72px / 對戰 + 場地 / ★ 40px），分「即將到來」asc + 「已過」desc 兩組；重用 `.card-fav-btn` 樣式做 ★；點 ★ call `toggleFavorite()` 後 row 立即消失 |
| `toggleFavorite()` 擴充 | 原本只 call `render()`（主賽程），現在同時 call `renderMyView()` 讓收藏 tab 即時更新 |
| 篩選區收藏/打卡 | `f-favorites-only` + `f-checkins-only` 兩個 checkbox → 單一 `f-ownership` 三段 segment（全部/只看收藏/只看已打卡），互斥、跟「場次狀態」segment 同樣式 |
| Schema 遷移 | `userState.lastFilter` 加 `ownership` 欄位；`applyFilterToUI` 自動偵測舊 `favoritesOnly`/`checkinsOnly` 轉成 `ownership` — 不需清除使用者 localStorage |
| 篩選區排版 | 原本 5 個 filter-group 換行擠 2 列，壓縮 gap / padding / 字級讓 5 條件 + 重設按鈕排一列 |
| 位置 | 原初把 segment 放在賽程 section header 下方（頂部 chip 風格），user 要求回篩選區跟其他條件並排 — 改回篩選區內 |

#### 設計 exploration

- `design/favorites-filter-chip-preview.html` — 4 個 chip placement 變體（B1 獨立 toggle / B2 segment / B3 inline / B4 單主 chip）對比。選定 B2 後，進一步 user 要求放回篩選區（而非頂部 chip），所以最終形態是 B2 style 但留在原位置

#### 開發教訓

- **「位置」跟「樣式」是兩個獨立決策**：user 最初選 B 是「把篩選移到頂部」，但後來發現「segment 樣式」本身更重要，「放哪裡」反而傾向保守（留在篩選區跟其他條件並排）。教訓：呈現 preview 時把 placement 跟 visual style 分開讓使用者挑，不要綁在同一個變體裡
- **schema 遷移主動處理**：這次 `lastFilter` 從 2 欄位（`favoritesOnly`/`checkinsOnly`）變成 1 欄位（`ownership`），在 `applyFilterToUI` 裡做 fallback 偵測舊格式 → 使用者的 localStorage 不需要清掉。Phase 1 `schema: v1` 設計當初就有預留空間容這類擴充，這次實際運用到
- **排版一排的要求要算總寬**：user 要求「條件排一排」，要先計算現有各 filter-group 總寬是否容得下 container（~1120px 內）。算完發現要壓縮 `.toggle-btn` padding（從 8/20 縮到 6/12）+ select min-width 才能擠入。教訓：排版類需求先拿尺量，不要憑感覺縮 padding
