...
- **顯示更多（2026-08-04）**：清單只渲染前 `NEWS_PAGE_SIZE`（50）則，其餘由底部「顯示更多（還有 N 則）」按鈕每次 +50 載出（`newsShownLimit`）。收合列筆數仍是**篩選後總數**。任何篩選變動（球隊/時間/關鍵字/重設）或重進新聞頁都經 `rerenderNewsFromFilter()` 把 `newsShownLimit` 重置回 50。按鈕用事件委派綁在 `#news-view-body`（每次 render 會重建內容）。
...
- **保留量 vs 上限**：三來源後量暴增，`$MaxItems=100` 上限現在只涵蓋約 4 天（2026-08-04 觀察：單日可達 50+ 則），`$KeepDays=45` 形同失效（舊聞被 100 則上限擠掉而非 45 天）。目前定位為「滾動近況 feed」；若要保留更多歷史再調高 `$MaxItems`（需搭配「顯示更多」避免頁面過長）。
