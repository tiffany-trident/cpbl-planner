...
- [x] 介面呈現：放 card-top 列（時間與球場之間），hourly 前綴「18時」、daily 前綴「全日」區分預報時段
- [x] 降雨率警示：≥ 50% 文字轉紅色 `#C25B5B`，暫不加卡片背景色
- [x] 點擊展開逐時預報：天氣 pill 可點擊彈出 popover，顯示開賽前 3 小時到開賽後 4 小時（共 8 格）逐時天氣。桌面版為浮層（自動依空間上下翻轉），手機版（<768px）為底部 sheet。點 pill 再一次 / ESC / 點浮層外皆可關閉。僅 72 小時內場次可用，不另打 API（沿用既有逐時陣列）。
