...
- **想看的隊伍（單選下拉）**：預設「全部（完整賽程）」；設有主隊時，主隊以 `⚾ …（主隊）` 釘在選單最上（全部之後、分隔線隔開）。主隊變更會即時重建選單（`rebuildTeam1Options()`）
- **對手（多選 logo chip）**：圓形隊徽 chip，可多選；active 加隊色外環 + ✓ 角標。清單**自動排除**目前「想看的隊伍」（不跟自己比，`renderTeam2Chips()`）
- **球場／月份下拉**：dropdown 選擇
- **週末／週末+國定假日**：toggle 篩選，台灣 2026 國定假日內建
- **場次狀態三段切換**：未來場次（預設）／全部／已結束
- 以上全部收在**單一「篩選」卡**內（原「隊伍篩選 / 條件篩選」兩張卡合併，2026-07-16）
- **篩選記憶**：`lastFilter.team1` 存 0–1 元素陣列（相容舊多選 schema，還原時取第一個）
...
- 三個分頁：`上半季`、`下半季`、`全年`，各自套用相同排行邏輯、只餵入不同場次子集
- **半季歸屬一律用賽事編號 sno 判斷**（`sno ≤ 180` 上半季、`≥ 181` 下半季）；補賽沿用原 sno，故自動算回原半季，比用日期切穩（常數 `FIRST_HALF_MAX_SNO`）
- **預設分頁依當天日期**：今天 < 下半季開打日 → 上半季（含季中空檔）；下半季開打日 ≤ 今天 ≤ 球季最後一場 → 下半季；季末後 → 全年
- 六隊一律先補 0-0-0，半季尚未開打時表格仍完整呈現
- caption 說明賽制：半季勝率第一鎖定季後賽門票；全年勝率決定季後賽種子序與外卡；同隊包辦兩冠時改由全年第 2、3 名爭挑戰賽
- **半季冠軍標章**：以**數學封王**判定（`clinchedHalfChampion`）——第一名在「自己剩餘全輸、對手剩餘全贏」最壞情況下勝率仍嚴格高於所有對手時，即於勝率第一列顯示「上／下半季冠軍」，不必等該半季每場打完；尚未封王則只顯示「暫居第一 領先」，不發冠軍。剩餘場次以 sno 去重計，估計保守（不會誤發）
